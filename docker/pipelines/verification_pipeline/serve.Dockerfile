# Verification pipeline serving image — extends base, adds FastAPI + uvicorn.
ARG BASE_IMAGE=base
FROM ${BASE_IMAGE}
...
COPY app/verification_pipeline/ /app/app/verification_pipeline/
...
EXPOSE 8080
CMD ["/app/.venv/bin/uvicorn", "app.verification_pipeline.app:app", "--host", "0.0.0.0", "--port", "8080"]
