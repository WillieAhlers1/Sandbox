...
class TestDeployModelInstantiation:
    ...
    def test_deploy_model_instantiation(self):
        """DeployModel creates with model_name and sensible defaults."""
        dm = DeployModel(model_name="regression")
        assert isinstance(dm, BaseComponent)
        assert dm.model_name == "regression"
    ...
    def test_deploy_model_has_no_serving_image_fields(self):
        """Removed fields must not appear on DeployModel."""
        fields = DeployModel.model_fields
        assert "serving_container_image" not in fields
        assert "serving_dockerfile" not in fields
        assert "model_uri" not in fields
...
class TestDeployModelLifecycle:
    ...
    def test_execute_calls_run(self, mock_run_deploy: MagicMock):
        ...
        dm = DeployModel(model_name="regression", project="test-project", region="us-central1")
    ...
    def test_subclass_run_override(self, mock_run_deploy: MagicMock):
        ...
        cd = CustomDeploy(model_name="regression", project="test-project", region="us-central1")
        cd.execute()
