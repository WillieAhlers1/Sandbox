...
def _provision_terraform(pipeline_dir: Path) -> None:
    """
    Automatically run Terraform to provision GCP resources.
    Uses terraform.tfvars if it exists, otherwise skips.
    """
    terraform_dir = pipeline_dir / "terraform"
    tfvars = terraform_dir / "terraform.tfvars"
    
    # Skip if no terraform directory or tfvars
    if not terraform_dir.exists() or not tfvars.exists():
        return
    
    try:
        console.print(f"[cyan]Provisioning infrastructure[/cyan] — {terraform_dir}")
        ...
        try:
            ...
            if result.returncode != 0:
                console.print(f"[yellow]terraform init warning:[/yellow] {result.stderr}")
            
            # Apply with auto-approve (no prompts)
            result = subprocess.run(
            ...
            if result.returncode != 0:
                err_console.print(f"[red]terraform apply failed:[/red] {result.stderr}")
                raise typer.Exit(1)
            
            console.print(f"[green]✓ Infrastructure provisioned[/green]")
            
        finally:
            os.chdir(original_cwd)
            
    except FileNotFoundError:
        err_console.print("[red]Error:[/red] terraform not installed. Run: brew install terraform")
        raise typer.Exit(1)
    except Exception as exc:
        err_console.print(f"[red]Provisioning error:[/red] {exc}")
        raise typer.Exit(1)
...
def _run_vertex(
    ...
    for name in targets:
        ...
        # Step 1: Provision infrastructure (BQ, FS SQL, Docker, Artifact Registry)
        _provision_terraform(pipeline_dir)
        
        # Step 2: Load and compile pipeline
        pipeline_def = _load_pipeline(pipeline_dir)
