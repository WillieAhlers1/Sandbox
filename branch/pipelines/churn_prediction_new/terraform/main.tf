# Main Terraform configuration for churn_prediction_new pipeline provisioning
...
resource "google_bigquery_dataset" "churn_prediction_new_dataset" {
  dataset_id           = var.bigquery_dataset_id
  friendly_name        = "churn_prediction_new Dataset"
  description          = "Dataset for churn_prediction_new pipeline"
  location             = var.region
...
resource "google_bigquery_job" "execute_bq_sql" {
  ...
  location         = var.region
  labels           = merge(var.common_labels, { pipeline = "churn_prediction_new", managed_by = "terraform" })
  ...
  depends_on = [google_bigquery_dataset.churn_prediction_new_dataset]
}
...
resource "google_artifact_registry_repository" "churn_prediction_new_repo" {
  location      = var.region
  repository_id = replace("churn_prediction_new", "_", "-")
  description   = "Docker images for churn_prediction_new pipeline"
  format        = "DOCKER"
...
resource "null_resource" "docker_build" {
  provisioner "local-exec" {
    command     = "docker build -t ${var.region}-docker.pkg.dev/${var.project_id}/${replace("churn_prediction_new", "_", "-")}/trainer:latest -f trainer/Dockerfile ."
  ...
  depends_on = [google_artifact_registry_repository.churn_prediction_new_repo]
}
...
resource "null_resource" "docker_push" {
  provisioner "local-exec" {
    command     = "docker push ${var.region}-docker.pkg.dev/${var.project_id}/${replace("churn_prediction_new", "_", "-")}/trainer:latest"
    working_dir = "${path.module}/.."
    interpreter = var.shell_interpreter
  }
