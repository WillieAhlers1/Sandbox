# Terraform - Infrastructure as Code for churn_prediction Pipeline

Automatic provisioning of GCP resources for the churn_prediction pipeline.
...
```bash
gml run churn_prediction --vertex --sync
```
...
Running `gml run churn_prediction` will execute all 4 files with no additional configuration needed.
...
region                    = "us-east4"
bigquery_dataset_id       = "dsci_churn_prediction_main"
...
All resources are labeled with `pipeline = "churn_prediction"` and `managed_by = "terraform"`.
...
4. **Run pipeline**: `gml run churn_prediction --vertex --sync`

---

**Important**: 
- ✅ terraform.tfvars **auto-created** from framework.yaml
- ✅ `gml run churn_prediction --vertex --sync` runs **terraform apply -auto-approve** (no prompts)
